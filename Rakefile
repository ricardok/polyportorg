...
desc 'Push to staging.polyport.org'
task :staging do
  require 'colored'
  ...
  File.open('CNAME', 'w') { |file| file.write 'staging.polyport.org' }
  ...
  system 'git remote add staging git@github.com:ricardok/staging.polyport.org.git'
...
task :deploy do
  ...
  system 'git push -f deploy gh-pages master:gh-pages'
